$(document).ready(function(){
    $("form[id^='frm_cancelar_reservaciones']").on('submit',(function(e){ // "e" IDENTIFICADOR DE ACCION PARA CAPTURAR EVENTO DE FORMULARIO
        let IdReservacion = $(this).find('#txtIdReservacion').val(); // IDENTIFICADOR UNICO RESERVACION
        // PARA SELECCIONAR MULTIPLES FORM CON DISTINTOS ID
        let ComentarioCancelacion = $(this).find('textarea[name="txtcomentariocancelacion_reservacion"]').val(); // COMENTARIO CANCELACION
        if(IdReservacion === ""){
