$(document).ready(function(){  
    // #bloquear-usuarios IDENTIDICADOR DE ID -> ACCION A EJECUTAR
   ...
   function BloquearUsuario(IDUsuarios){ 
    swal({
        ...
        text: "¿Realmente desea bloquear este usuario?",
        icon: "warning",
        buttons: true,
        dangerMode: true,
    })
    .then((willDelete) => {
        if (willDelete) {
            $.ajax({
                url: '../Controlador/GestionesLaboratorios_Controlador.php?gestioneslaboratorios=envio-datos-bloquear-usuarios&idusuario='+IDUsuarios,
                type: 'POST',
                ...
                .done(function(respuesta){ // SI MODELO EJECUTA PETICION, REALIZA PETICION
                  if(respuesta=="OK"){
                      AlertaUsuarioMostrar("Usuario Bloqueado","Su petición se ha realizado con éxito","success");
