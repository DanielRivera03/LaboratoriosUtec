...
$(document).ready(function () {
  $("#frm_actualizacionfinal_reservaciones").on('submit', function (e) {
    // "e" IDENTIFICADOR DE ACCION PARA CAPTURAR EVENTO DE FORMULARIO
    var CodigoUnicoReservacion = $('#txtCodigoIdentificadorUnico').val(); // CODIGO UNICO RESERVACIONES

    var EstadoReservacion = $('#sltestadoinicial_reservaciones').val(); // ESTADO DE RESERVACIONES

    var ComentarioFinalReservacion = $('#txtcomentariofinal_reservaciones').val(); // COMENTARIO FINAL RETROALIMENTACION RESERVACIONES
    ...
    if (ComentarioFinalReservacion === "") {
      Notificaciones_NotifyDanger("<i class='icofont icofont-marker-alt-3'></i> Campo incompleto", "<i class='icofont icofont-ui-flash-light'></i> El comentario de retroalimentación final es requerido");
      return false;
    } else {
      e.preventDefault(e); // "e" IDENTIFICADOR DE ACCION PARA CAPTURAR EVENTO DE FORMULARIO

      jQuery.ajax({
        url: "../Controlador/GestionesLaboratorios_Controlador.php?gestioneslaboratorios=envio-datos-actualizacion-final-reservaciones-laboratorios",
        type: "POST",
        dataType: 'json',
        ...
        success: function success(resultado) {
          setTimeout(function () {
            ...
            setTimeout(function () {
              location.href = "../Controlador/GestionesLaboratorios_Controlador.php?gestioneslaboratorios=consulta-reservaciones-aprobacion-inicial";
            }, 2500);
          }, 3500);
        },
...
//# sourceMappingURL=AJAX_envio-datos-actualizacion-final-reservaciones.dev.js.map
