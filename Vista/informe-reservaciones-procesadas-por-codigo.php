...
$pdf->AliasNbPages();
$pdf->AddPage('L', array(300, 888));
// CONTENIDO DE REPORTE [DOCUMENTO]
$pdf->SetFont('Arial','',13);
$pdf->Ln(20);
$pdf->MultiCell(858,5,utf8_decode("San Salvador, ".utf8_encode(strftime("%A %d de %B de %Y")).""),'','R');
$pdf->Ln(15);
$pdf->MultiCell(858,5,utf8_decode("Estimado(a) ".$PrimerNombre[0].", usted ha solicitado el respectivo informe de reservaciones procesadas en el rango de fechas desde ".$FechaInicioReservacion." hasta ".$FechaFinReservacion." bajo el código de reservación ".$CodigoReservacion.". Tome en consideración si los respectivos módulos aparecen vacíos, quiere decir que ese laboratorio en cuestión no maneja el servicio por módulos. A continuación le mostramos el detalle general:"));
$pdf->Ln(5);
...
foreach ($consulta as $filas) {
    ...
    $pdf->Cell(90, 10, utf8_decode($filas['nombre_titular']), 0, 0, 'C', 1);
    $pdf->Cell(40, 10, utf8_decode($filas['fechainicio']), 0, 0, 'C', 1);
    $pdf->Cell(40, 10, utf8_decode($filas['fechafin']), 0, 0, 'C', 1);
